...
class Loader(object):
    def __init__(self, dir_original, dir_segmented, init_size=(img_width, img_height), one_hot=True):
    ...
    #只预测用
    def load_predict_image(self):
        data_size = int(len(self._data.images_original))
        predict_set = self._data.perm(0, data_size)
        return predict_set
    # def import_data_predict(dir_original,init_size=None):
    #     paths_original= Loader.generate_paths_pre(dir_original)
    #     images_original= Loader.extract_images_pre(paths_original, init_size)
    #     return DataSetPre(images_original)
    # def generate_paths_pre(dir_original):
    #     paths_original = glob.glob(dir_original + "/*")
    #     paths_original = list(map(lambda filename: dir_original + "/" + filename + ".png"))
    #     return paths_original
    # def extract_images_pre(paths_original, init_size, one_hot):
    #     images_original = []
    #     # Load images from directory_path using generator
    #     print("Loading Predict images", end="", flush=True)
    ...
    def import_data(dir_original, dir_segmented, init_size=None, one_hot=True):
        ...
        return DataSet(images_original, images_segmented, palette,
                       augmenter=ia.ImageAugmenter(size=init_size,
                                                   class_count=255
                                                    #class_count = len(DataSet.CATEGORY)
                                                   ))
    @staticmethod
    ...
    def extract_images(paths_original, paths_segmented, init_size, one_hot):
        ...
        images_segmented = np.where(images_segmented == 255,
                                    255,
                                    #len(DataSet.CATEGORY) - 1,
                                    images_segmented)
        ...
        if one_hot:
            ...
            identity = np.identity(255, dtype=np.uint8)
...
'''
#只预测用
class LoaderPredict(object):
    def __init__(self, dir_original, paths_segmented, init_size=(img_width, img_hight)):
        self._data = LoaderPredict.import_data_predict(dir_original,paths_segmented, init_size)
    def get_all_dataset(self):
        return self._data
    #只预测用
    def load_predict_image(self):
        ...
        predict_set = self._data #.perm(0, data_size)
        return predict_set
    def import_data_predict(dir_original, paths_segmented ,init_size=None):
        paths_original= LoaderPredict.generate_paths_pre(dir_original)
        images_original= LoaderPredict.extract_images_pre(paths_original, init_size)
        image_sample_palette = Image.open(paths_segmented)
        palette = image_sample_palette.getpalette()
        #print(palette)

        return DataSetPre(images_original, palette)

    def generate_paths_pre(dir_original):
        paths_original = glob.glob(dir_original + "/*")
       # paths_original = list(map(lambda filename: dir_original + "/" + filename + ".png"))
        return paths_original
    def extract_images_pre(paths_original, init_size):
        images_original = []
        # Load images from directory_path using generator
        print("Loading Predict images", end="", flush=True)
        for image in LoaderPredict.image_generator(paths_original, init_size, antialias=True):
            images_original.append(image)
            if len(images_original) % 200 == 0:
                print(".", end="", flush=True)
        print(" Completed", flush=True)
    ...
    def image_generator(file_paths, init_size=None, normalization=True, antialias=False):
        ...
        for file_path in file_paths:
            if file_path.endswith(".png") or file_path.endswith(".jpg"):
                ...
                image = Image.open(file_path)
                # to square
                image = LoaderPredict.crop_to_square(image)
                # resize by init_size
                if init_size is not None and init_size != image.size:
                    if antialias:
                        image = image.resize(init_size, Image.ANTIALIAS)
...
class DataSetPre(object):
    def __init__(self, images_original, image_palette):
